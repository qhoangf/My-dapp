...
import { contractABI, contractAddress, marketplaceAddress, marketplaceABI } from "../../../contract";
...
export default function CancelListing() {
    ...
    async function cancelList(){
        ...
        if(Number(check) == Number(accounts[0])){
            const requestCancel = await marketplaceContract.methods.cancelSignature(contractAddress, tokenId, hash, _v, _r, _s).send({from: accounts[0]})
            .catch(e =>{
                if(e.code == 4001){
    ...
    const btn = () =>{
        return(
            <>
                <button className={styles.cancelbtn} onClick={closePopup}>Go back</button>
                <button className={styles.updatebtn} onClick={cancelList}>Continue</button>
            </>
        )
    }
    ...
    const btnWaiting = () =>{
        return(
            <>
                <button disabled className={styles.waitingbtn} >Go back</button>
  ...
  return (
    <div id='myModalCancel' className={styles.modal}>
        <div className={styles.modalcontent}>
            ...
            <div>
                <p>This will cancel your listings. You will also be asked to confirm this cancelation from your wallet.</p>
            </div>
